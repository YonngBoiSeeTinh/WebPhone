...
const TimeKeeping = () => {
  ...
  const refech = queryTimeKeeping
  const querydataTimeKeeping = queryTimeKeeping.data || [];
  const listTimeKeeping = querydataTimeKeeping.filter((item) =>{
    const itemDate = new Date(item.date).toISOString().split('T')[0]; // Chuyển item.date về định dạng "YYYY-MM-DD"
    ...
    <div className="timekeeping_container">
      ...
      <div className="timekeeping_list">
        {currentCus.map((item, index) => {
           const timeKeepingRecord = listTimeKeeping.find(
