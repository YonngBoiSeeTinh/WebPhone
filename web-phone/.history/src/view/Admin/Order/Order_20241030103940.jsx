...
const Order = ({  setNotify}) => {
  ...
  useEffect(() => {
    // Chỉ cập nhật thông báo nếu có đơn hàng mới chưa xác nhận
    if (listNonAccept.length > 0) {
        const message = `Có ${listNonAccept.length} đơn hàng mới chưa xác nhận`;
        // Kiểm tra xem thông báo đã tồn tại chưa
        setNotify(prev => {
            if (!prev.includes(message)) {
                return [...prev, message]; // Chỉ thêm nếu thông báo chưa tồn tại
            }
            return prev; // Không thay đổi nếu thông báo đã có
        });
    }
