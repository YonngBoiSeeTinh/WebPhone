...
function UserPage({ setIsSignIn,setAlertMessage,setShowAlert, setType }) {
    ...
    const handleSaveAddress = async (newAddress) => {
        try {
            const response = await axios.put(`http://localhost:3001/api/user/update/${user.id}`, { address:newAddress });
            console.log(user.id);
            console.log(response);
            if (response.data.status === 'OK') {
                
                dispatch(updateUser({ ...user, address:newAddress })); // Cập nhật Redux
