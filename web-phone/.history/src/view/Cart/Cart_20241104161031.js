    ...
    const handleOrder = async () => {
      ...
      orderProducts.map(async (item) => {
        try {
          ...
          // Find the color data that matches the item's color
          const colorData = productData.colors.find((color) => color.name === item.color);
      
          if (colorData) {
            if (item.amount > colorData.countInstock) {
              setAlertMessage("Số lượng sản phẩm trong kho không đủ.");
              setType("warning");
              setShowAlert(true);
              return;
            }
          } else {
            setAlertMessage("Màu sắc không tồn tại.");
            setType("error");
            setShowAlert(true);
          }
        } catch (error) {
          console.error("Error fetching product details:", error);
          setAlertMessage("Lỗi khi tải thông tin sản phẩm.");
          setType("error");
          setShowAlert(true);
        }
      });
