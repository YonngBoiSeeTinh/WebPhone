...
const Order = ({ setAlertMessage, setShowAlert, setType }) => {
  ...
  const [searchQuery, setSearchQuery] = useState(""); // Tạo state cho ô tìm kiếm
  ...
  // Lọc đơn hàng chưa được duyệt
  let listNonAccept = listOrder.filter((item) => !item.accept);

  // Nếu có tìm kiếm, lọc danh sách đơn hàng theo tên người dùng
  if (searchQuery) {
  ...
  const currentOrders = listNonAccept.slice(startIndex, startIndex + itemsPerPage);
